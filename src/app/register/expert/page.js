...
function Page() {
    ...
    const handleSubmit = async (e) => {
        ...
        setSuccess(false);
        try {
            // Construction du body attendu par createExpert
            const body = {
                firstName: form.firstName,
                lastName: form.lastName,
                email: form.email,
                password: form.password,
                adressrdv: form.adressrdv,
                specialties: [
                    {
            ...
            const res = await fetch("http://localhost:8000/api/experts/create", {
                method: "POST",
    ...
    return (
        <div className="min-h-screen flex justify-center relative">
            ...
            <div className="mt-[18vh] flex flex-col items-center">
                <span className={`${mont_low.className} text-noir text-[36px] mb-2`}>Devenez un praticien wicca</span>
                ...
                <div className="flex w-full items-center rounded-lg cursor-pointer">
                    <form className="flex flex-col items-center gap-y-3 w-full" onSubmit={handleSubmit}>
                        ...
                        <div className="flex flex-col w-full gap-y-2">
                            <span className={`${mont_low.className} text-noir/80 text-[15px]`}>Adresse de rendez-vous</span>
                            <input
                                type="text"
                                name="adressrdv"
                                placeholder=""
                                value={form.adressrdv}
                                onChange={handleChange}
                                className="border px-2 py-2 rounded-lg w-full text-sm text-black focus:outline-none focus:ring-1 focus:ring-maincolor focus:border-maincolor"
                                required
                            />
